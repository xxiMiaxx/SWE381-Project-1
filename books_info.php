...
<h1>Currently and history of <?php echo $user['Name'] ?> :</h1>
<script>
 function sendNot(u,b){
     $.get("send_noti.php?u_id=" + u + "&b_id=" + b , function( data ) {
...
<?php }else{ ?>
  ...
  <tbody>
    ...
    <tr>
        <td><?php echo $cur['title'] ?></td>
        <td><?php echo $cur['cur_due'] ?></td>
        <td><?php echo $cur['issued_date'] ?></td>
        <td>
      <button onclick="sendNot('<?php echo $user['ID'] ?>','<?php echo $cur['id'] ?>')">Send Notification</button>
        </td>
    </tr>
      <?php } ?>
  </tbody>
...
<a class="btn btn-default" href="./memebrs2.php">back</a> 
